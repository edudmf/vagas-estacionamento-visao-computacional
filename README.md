...
O dataset de origem só tem anotação de bounding box, sem máscara de segmentação. Diante do prazo do projeto, optamos por não anotar uma amostra manualmente e, em vez disso, adaptamos um modelo de segmentação já pré-treinado (YOLOv8n-seg, pré-treinado no COCO) para o mesmo domínio (imagens e vídeo do estacionamento), restringindo a inferência às classes de veículo do COCO (carro, ônibus, caminhão) e ajustando o limiar de confiança para 0,10. Essa é uma segmentação de veículos, não de status de vaga (busy/free), e não passou por fine-tuning no dataset do projeto. Essa decisão e sua limitação estão detalhadas na seção 6.
